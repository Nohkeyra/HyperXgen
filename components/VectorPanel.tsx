...
export const VectorPanel: React.FC<VectorPanelProps> = ({
  ...
  const PRESETS = useMemo(() => {
    let presetsToRender: PresetCategory[] = [
      PRESET_REGISTRY.VECTOR.signature, 
  ...
  const handleGenerate = async () => {
    ...
    const isImageOnly = !effectivePrompt && !!uploadedImage;
    
    const combinedPrompt = [activePreset?.prompt, effectivePrompt].filter(Boolean).join('. ');
    const synthPrompt = isImageOnly ? "" : combinedPrompt || "VECTOR_CORE";
    
    processingRef.current = true;
    transition(dna ? "DNA_STYLIZE_ACTIVE" as any : isImageOnly ? "DETECTING_SILHOUETTE" : "DEVOURING_BUFFER", true);
    setIsValidationError(false);
    
    try {
      const result = await synthesizeVectorStyle(synthPrompt, uploadedImage || undefined, kernelConfig, dna || undefined);
      setGeneratedOutput(result);
      transition("LATTICE_ACTIVE");
      onSaveToHistory({
        id: `vector-${Date.now()}`,
        name: effectivePrompt.slice(0, 15) || (activePreset?.name || dna?.name || "Vector Synthesis"),
        description: synthPrompt || "Subject-aware blueprint",
  ...
  const SidebarContent = (
    <>
      <SidebarHeader 
        ...
        title="Vector_Engine" 
        version="Deterministic Synthesis v4.8"
